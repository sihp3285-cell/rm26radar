...
{
public:
    /** 加载 DetectionConfig/DetectPipeline，创建图像订阅、三个 publisher 和 ROI 重载服务。 */
        ...
        : Node("detect_node", options)
    {
        ...
        cfg_ = std::make_unique<DetectionConfig>(config_dir, declare_parameter<std::string>("roi_path", (std::filesystem::path(config_dir)/"outpost_roi.yaml").string()));
        const auto model_dir=declare_parameter<std::string>("model_dir", "");
        if (!model_dir.empty()) {
            for(auto* path : {&cfg_->model.modelPath, &cfg_->model.armorModelPath, &cfg_->model.classifyModelPath, &cfg_->model.airplaneModelPath})
...
private:
    ...
    // DetectionConfig 必须比 DetectPipeline 活得久：pipeline 内部保存 cfg_ 的引用，并据此
    // 读取模型开关/ROI。成员按声明逆序析构，因此此处先声明 DetectionConfig、后声明 pipeline。
    ...
    /** Trigger 回调：仅从 outpost_roi.yaml 更新 DetectionConfig 中 ROI，不重建 TensorRT 模型。 */
                   ...
                   std_srvs::srv::Trigger::Response::SharedPtr response)
    {
        try {
            ...
            std::string outpost_yaml = get_parameter("roi_path").as_string();
