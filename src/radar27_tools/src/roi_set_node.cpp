...
public:
    ...
    ROISetNode() : Node("roi_set_node")
    {
        ...
        declare_parameter<std::string>("calibration_path", "calib_result.yaml");
...
private:
    ...
    bool isCalibValid()
    {
        ...
        std::string calib_yaml = get_parameter("calibration_path").as_string();
    ...
    std::pair<bool, std::string> doROISet()
    {
        try {
            ...
            RCLCPP_ERROR(get_logger(), "ROI 框定失败: %s", e.what());
            return {false, std::string("ROI 框定失败: ") + e.what()};
        }
    }

    std::pair<bool, std::string> doROISetImpl()
    {
    ...
    bool saveROIResult(const cv::Rect& roi)
    {
        try {
            ...
            const std::string temporary_path=outpost_roi_path_+".tmp";
            std::ofstream fout(temporary_path);
            if (!fout.is_open()) {
            ...
            fout << node;
            fout.flush();
            if (!fout.good()) throw std::runtime_error("Failed to write configuration");
            fout.close();
            std::filesystem::rename(temporary_path, outpost_roi_path_);
...
/** HighGUI 必须始终在主线程运行；嵌套服务由各自的临时 executor 等待。 */
