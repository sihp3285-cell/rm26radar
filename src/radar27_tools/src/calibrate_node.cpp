...
private:
    ...
    {
        try {
            return doCalibrationImpl();
        } catch (const std::exception& e) {
            RCLCPP_ERROR(get_logger(), "标定失败: %s", e.what());
            return {false, std::string("标定失败: ") + e.what()};
        }
    }

    std::pair<bool, std::string> doCalibrationImpl()
    {
                         ...
                         const cv::Mat& R, const cv::Mat& T)
    {
        try {
            ...
            const std::string temporary_path=calib_result_path_+".tmp";
            ...
            fout.close();
            std::filesystem::rename(temporary_path, calib_result_path_);
