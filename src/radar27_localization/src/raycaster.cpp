 ...
 * Open3D CUDA 查询在定位共享库内串行化；批量接口减少逐框构造 Tensor
 * 与场景调用开销。无命中以非有限/约定结果交给上层过滤，不在本层猜测地面高度。
 */
#include <radar27_localization/raycaster.hpp>
#include <mutex>
namespace { std::mutex raycast_mutex; }
...
cv::Point3f Raycaster::pixelToWorld(const cv::Point2f& pixel, 
                             ...
                             const cv::Mat& R_inv, const cv::Mat& T) const
        {
            ...
            float t_hit;
            {
                std::lock_guard<std::mutex> cudaLock(raycast_mutex);
