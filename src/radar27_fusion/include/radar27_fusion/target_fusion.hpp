...
inline radar27_interfaces::msg::FusedTargetArray fuse(
    ...
    output.targets.resize(rm_field::kRobotSlotCount);
    const double prior_age = prior
        ? (stamp_ns(world.header.stamp) - stamp_ns(prior->header.stamp)) * 1e-9 : -1.0;
    const bool prior_fresh = prior && prior->calibration_version == world.calibration_version && prior->model_enabled && prior_age >= 0.0 &&
