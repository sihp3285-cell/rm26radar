...
/** Render-only map observer. Business outputs are owned by radar27_decision.
 * /display_flip controls presentation; /match_state controls opponent filtering.
 */
...
public:
        ...
        : Node("map_node", options)
    {
        ...
        cfg_ = std::make_unique<DisplayConfig>(config_dir);
        ...
        flip_team_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "/display_flip", rclcpp::QoS(1).reliable().transient_local(),
        ...
        match_sub_=create_subscription<radar27_interfaces::msg::MatchState>("/match_state",rclcpp::QoS(1).reliable().transient_local(),[this](radar27_interfaces::msg::MatchState::ConstSharedPtr m){own_team_red_=m->own_team==robot_id::RED;});
...
private:
        ...
        const radar27_interfaces::msg::WorldTargetArray& targets)
    {
        ...
        for (const auto& prediction : prior->predictions) {
            ...
            const int opponent_team = robot_id::opponent_team_for_view(own_team_red_);
            ...
            if (prediction.role_class_id >= 0 &&
                prediction.role_class_id < static_cast<int>(cfg_->class_names.size())) {
                label << cfg_->class_names[prediction.role_class_id];
            } else {
        ...
        try {
            ...
            for (size_t i = 0; i < msg->targets.size(); ++i) {
                ...
                if (t.class_id == robot_id::ARMOR && t.is_dead) {
                    ...
                    mp.isDead = true;
                    if (robot_id::ARMOR >= 0 && robot_id::ARMOR < static_cast<int>(cfg_->class_names.size())) {
                        mp.label = cfg_->class_names[robot_id::ARMOR];
                    } else {
                        mp.label = "dead";
                    }
                ...
                mp.isDead = t.is_dead;
                if (t.stable_class_id >= 0 && t.stable_class_id < static_cast<int>(cfg_->class_names.size())) {
                    mp.label = cfg_->class_names[t.stable_class_id];
                }
                mappoints.push_back(mp);

            }

            cv::Mat map_frame = radar_map_->drawMap(mappoints, cfg_->class_names);

            // ========== 前哨站叠加绘制（在 drawMap 之后） ==========
            const auto& outpostPts = cfg_->map.getOutpostMapPoints(own_team_red_);
    ...
    bool own_team_red_=false;
    rclcpp::Subscription<radar27_interfaces::msg::MatchState>::SharedPtr match_sub_;
    bool flip_team_ = false;
    std::unique_ptr<DisplayConfig> cfg_;
