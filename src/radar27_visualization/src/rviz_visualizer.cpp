...
namespace radar27_visualization::debug {
namespace {
        ...
        try {
            ...
            for (const auto& zone : zones) {
                ...
                if (effective_zone.vertices.size() == 1) {
                    ...
                    target.pose.position = canonical_to_world(vertex[0], vertex[1], flip_team, options_.world_z_toward_blue, 0.12);
                    ...
                    for (const auto& vertex : effective_zone.vertices) {
                        boundary.points.push_back(canonical_to_world(
                            vertex[0], vertex[1], flip_team, options_.world_z_toward_blue));
                    }
                    ...
                    for (const auto& triangle : triangulate_polygon(effective_zone)) {
                        for (const std::size_t vertex_index : triangle) {
                            ...
                            area.points.push_back(canonical_to_world(
                                vertex[0], vertex[1], flip_team, options_.world_z_toward_blue, 0.03));
                        }
                    }
                ...
                label.pose.position = canonical_to_world(
                    center_x, center_y, flip_team, options_.world_z_toward_blue, 0.35);
                label.scale.z = 0.22;
            ...
            for (int row = 0; row < rows; ++row) {
                for (int column = 0; column < columns; ++column) {
                    ...
                    const auto center = canonical_to_world(
                        canonical_x, canonical_y, flip_team, options_.world_z_toward_blue, world_y,
