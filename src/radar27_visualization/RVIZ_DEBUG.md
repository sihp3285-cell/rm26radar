...
RViz topic；关闭调试时不会加载 `RvizDebugNode`，PoseNode 只在开启调试时发布原始射线数据；RvizDebugNode 创建 Marker/TF
publisher 或复制逐帧调试数据。
...
```bash
rviz2 -d "$(ros2 pkg prefix radar27_visualization)/share/radar27_visualization/config/radar_debug.rviz"
```
...
`/match_state`，按当前敌方阵营执行与 Position Prior 相同的 canonical→field→world
...
子开关位于 `radar27_visualization/config/params.yaml`：`mesh`、`tracks`、`trajectories`、
`velocity`、`covariance`、`measurement_covariance`、`guess_candidates`、
`blind_zones`、`nav_grid`，以及 RvizDebugNode 的
...
启动时还需传入 `model_dir`，视频模式需传入 `video_path`；详见根目录 ARCHITECTURE.md。
